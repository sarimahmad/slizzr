...
export default class payoutMethod extends Component {
    constructor() {
        super();
        this.state = {
            payoutData: [{ id: 1, name: "TD Bank", card_number: '#********101' },
                            ...
                            <View>
                                <View style={styles.blockView}>
                                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                        ...
                                        {this.state.editType && <Image style={{ width: 23, height: 22, marginLeft: 8 }}
                                            source={require('../../assets/Slizzer-icon/cross.png')} />}
                                        <View>
                                            <Text style={[styles.textView, { color: item.id !== 1 ? '#494949' : '#F818D9' }]}>{item.name}</Text>
                                            <Text style={[styles.textView, { fontSize: 12, color: item.id !== 1 ? '#494949' : '#F818D9' }]}>{item.card_number}</Text>
                                        </View>
